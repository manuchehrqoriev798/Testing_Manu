...
// CustomNode component (unchanged except for styling via CSS)
...
const TreeVisualizerContent = ({ onActivate, onBack }) => {
  ...
  const NODE_WIDTH = 50; // Used for spacing calculations, not node size

  // Calculate the width required for a subtree at a given level
  ...
  // Recalculate all node positions after a change
  const updateNodePositions = useCallback((nodes) => {
    ...
    const updatePositions = (nodeId, x, level) => {
      ...
      const leftChildNodes = node.data.leftChildId ? updatePositions(node.data.leftChildId, x - spacing, level + 1) : [];
      const rightChildNodes = node.data.rightChildId ? updatePositions(node.data.rightChildId, x + spacing, level + 1) : [];

      return [updatedNode, ...leftChildNodes, ...rightChildNodes];
    };

    return updatePositions(root.id, root.position.x, 0);
  }, [getSubtreeWidth, LEVEL_HEIGHT]);
  ...
  const addChild = useCallback((parentId, side) => {
    setNodes(currentNodes => {
      ...
      // Temporary position, will be updated by updateNodePositions
      ...
      // Recalculate all positions to prevent overlap
      return updateNodePositions(updatedNodes);
    });
  }, [updateNodePositions, handleNodeLabelChange]);
